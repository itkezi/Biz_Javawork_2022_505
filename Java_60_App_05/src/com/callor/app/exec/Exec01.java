...
/*
 * 배열, 리스트에 담겨있는 데이터들을 어떤 항목, 값을 기준으로 정렬할 경우가 많다
 * DBMS와 같은 데이터 관리 도구를 사용하는 경우는 SQL 명령으로 쉽게 정렬할 수 있다
 * 
 * 하지만, 배열 리스트에 담긴 데이터들은 그 양이 별로 많지 않기 때문에 코딩으로 데이터를 정렬할 수 있다
 * 
 ...
 * 
 * 이 순회(반복)을 전체 개수만큼 반복실행하여 데이터를 정렬하는 알고리즘
 * 코드가 매우 단순해서 조금만 살펴보면 쉽게 이해할 수 있다
 * insert 정렬과 비교했을 때 비교하는 방법에서 조금 차이가 있다
 * (쌤 의견) 큰 데이터가 아닐 경우는 insert 정렬에 비해 더 빠르다 
 * */
