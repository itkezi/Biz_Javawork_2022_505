...
public class Var_02 {
	public static void main(String[] args) {
		...
		// printf()를 이용하여 3개 변수에 담긴 데이터를 console로 출력하기
		// "%d + %d = %d" form String을 사용하여 출력할 모양을 지정하고
		// 각가 %d 는 Decimal(10진수)를 표현하는 키 단어이다.
		// 그런데 doNum1, doNum2, doSum 변수는 double타입이다.
		// double type 변수에 담긴 값을 %d 로 표현하려면
		// 내부에서 데이터 type 문제로 인한 중대한 문제가 발생한다
		// %d 는 정수만 표현할 수 있는데 실수값을 표현하라고 했기 때문에 발생하는 문제이다.
		

		// printf()를 사용하여 실수 데이터를 Console에 출력하려면 %f 기호를 사용하여 표현해야 한다.
		System.out.printf("\t%f + %f = %f\n", doNum1, doNum2, doSum);
		
		// %.2f : 정수 부분은 모두 표현하고 소수점 이하 2째자리까지 출력하라
		...
		// %8.2f : 전체 자릿 수를 8개로 하고 소수점 이하 2째까지 표현
		// 만약 데이터가 전체 자릿수보다 작으면 남은 부분을 공백으로 표현
		// 숫자를 오른쪽으로 정렬할 때 사용하는 방법
		// 8.2 : 정수 자릿수 + 소수점(1) + 소수점이하 2자리를 모두 합한 개수가 8개라는 표현
